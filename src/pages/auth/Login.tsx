    ...
    <>
      ...
      <LayoutSignIn>
        ...
        {StepType?.screen === "auth_email_phone" && (
          <>
            {StepType?.tabActive === "phone" && StepType?.step === 1 && (
              <HeaderLogoTitleLogin title="Enter your phone or email to log in" />
            )}
        ...
        <div className="p-2 flex justify-center">
          {!token ? <GoogleReCaptchaProvider reCaptchaKey="6Ldp7ToqAAAAAGqXWj-2hbS3T2M9qeOufJBrxQEO">
            <GoogleReCaptcha refreshReCaptcha={false} onVerify={handleVerify} />
          </GoogleReCaptchaProvider> : null}
        </div>
        <div className='flex justify-center mt-4'>
          <span className='text-[14px] text-[#000000]'>Don’t have an account? </span>
          <span className='text-[14px] text-[#5F4DD7] ml-1 cursor-pointer' onClick={() => navigate('/sign-up')}>Create one</span>
        </div>
      </LayoutSignIn>
    </>
  );
